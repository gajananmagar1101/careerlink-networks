    ...
    <label className="block">
      ...
      <span className="relative mt-2 block">
        ...
        <input
          ...
          {...props}
          className={clsx(
            'w-full rounded-md border bg-white px-3 py-2.5 text-sm text-ink transition placeholder:text-slate-400 focus:border-brand-500',
  ...
  ({ label, error, className, children, required, ...props }, ref) => (
    <label className="block">
      ...
      <select
        ...
        {...props}
        className={clsx(
          'mt-2 w-full rounded-md border bg-white px-3 py-2.5 text-sm text-ink focus:border-brand-500',
          error ? 'border-red-300' : 'border-line',
          className
        )}
      >
  ...
  ({ label, error, helper, className, required, ...props }, ref) => (
    <label className="block">
      ...
      <textarea
        ...
        {...props}
        className={clsx(
          'mt-2 min-h-28 w-full rounded-md border bg-white px-3 py-2.5 text-sm text-ink transition placeholder:text-slate-400 focus:border-brand-500',
          error ? 'border-red-300' : 'border-line',
          className
        )}
      />
      <span className="mt-1 flex justify-between text-xs text-muted">
        <span>{error ? <span className="font-medium text-red-700">{error}</span> : helper}</span>
